...
const followingUserCtrl = expressAsyncHandler(async (req, res) => {
  ...
  if(!targetUser)res.send("user not found");
...
const accountVerificationCtrl = expressAsyncHandler(async (req, res) => {
  ...
  const userFound = await User.findOne({
    accVerificationToken: hashedToken,
    accVerificationTokenExpires: { $gt: new Date() },
  });
  if (!userFound) throw new Error("Token expired, try again later");
  //update the proprt to true
  userFound.isAccVerified = true;
  userFound.accVerificationToken = undefined;
  userFound.accVerificationTokenExpires = undefined;
...
//  blockUser(req?.user);
  ...
  if(!req.file)res.send('not availavle');
...
module.exports={userRegisterCtrl,loginUserCtrl,
  fetchUsersCtrl,deleteUsersCtrl,
  fetchUserDetailsCtrl,userProfileCtrl,
  updateUserCtrl,updateUserPasswordCtrl,followingUserCtrl
